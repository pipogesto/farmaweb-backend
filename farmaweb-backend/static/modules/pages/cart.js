// static/modules/pages/cart.js
import { cart } from '../state.js'; 
import { SHIPPING_COST, FREE_SHIPPING_THRESHOLD } from '../config.js';

export async function initCartPage(main) {
    if (!main) return;

    // --- CORRECCIÓN: Rutas con /static/ ---
    if (cart.length === 0) {
        const response = await fetch('/static/pages/cart-empty.html');
        if (!response.ok) throw new Error(`No se pudo cargar cart-empty.html`);
        main.innerHTML = await response.text();
    } else {
        const response = await fetch('/static/pages/cart.html');
        if (!response.ok) throw new Error(`No se pudo cargar cart.html`);
        main.innerHTML = await response.text();
